...
![image-20210712143612920](redis.assets\image-20210712143612920.png)

> 包含: 可以包含多个配置文件的引入

![image-20210712143635033](redis.assets\image-20210712143635033.png)
...
缓存穿透：用户想要查一个数据，发现redis内存中没有，缓存没有命中，于是向持久层数据库查询。发现也没有导致本次查询失败，当用户很多的时候，缓存都没有命中，于是都去请求了持久层数据库，这会给数据库造成很大的压力，这时候就相当于出现了缓存穿透
...
- 布隆过滤器
  ...
  ![image-20210712135739955](redis.assets\image-20210712135739955.png)

- 缓存空对象

  当存储层不命中后，即使返回的空对象，也缓存起来，同时会设置一个过期时间，之后再访问这个数据将会从缓存中获取，保护后端数据源

  - 存在的问题：空值被缓存，需要更多的空间存储键，
  - 存在缓存层和存储层数据一致性问题，对一致性要求高的业务会有影响



> 缓存击穿：量太大，缓存过期（微博同时访问一条）

- 设置热点数据永不过期

- 加互斥锁

  分布式锁：使用分布式锁，保证对于每个key同时只有一个线程去查询后端服务，其他线程没有获得分布式锁的权限，因此只需等待。这种方式将高并发的压力转移到分布式锁，因此对分布式锁的考验很大
...
- 数据预热
  - 在正式部署之前，先把可能的数据先预先访问一遍，这样部分可能大量访问的数据就会加载到缓存中。在即将发生大并发访问前，手动触发加载缓存不同的key，设置不同的过期时间，让缓存失效的时间点尽量均匀
